...
public class ProjectTabAction extends AbstractBaseProjectAction {
	...
	public String save () {
		...
		if (initialSave && getProject()	.getExperiment()
										.getPrimaryInvestigator() == null) {
			...
			ExperimentContact pi = new ExperimentContact(	getExperiment(),
															new Person(UsernameHolder.getCsmUser()),
															Arrays.asList(	piRole,
																			mainPocRole));
	 ...
	 * 
	 * @param e
	 *            the exception containing information about the inconsistency
	 */
	protected void handleInconsistentStateError ( InconsistentProjectStateException e)
	{
		List<String> args = new ArrayList<String>();
		args.add(StringUtils.abbreviate(getProject().getExperiment().getTitle(),
										TRUNCATED_TITLE_WIDTH));
		if (e.getReason() == Reason.INCONSISTENT_ARRAY_DESIGNS) {
			...
			addFieldError(	"project.experiment.arrayDesigns",
							getText("project.inconsistentState." + e.getReason()
																	.name()
																	.toLowerCase(),
									args));
			...
			addActionError(getText("project.inconsistentState." + e	.getReason()
																	.name()
																	.toLowerCase()));
		}
	}
	...
	/**
	 * Helper method for checking whether the project was correctly loaded and
	 * has at least read rights. if not, returns the result name that the action
	 * should redirect to; otherwise returns null. Action methods should call
	 * this helper method first; if it returns a non-null result, they should in
	 * turn return this result immediately, otherwise they can proceed with
	 * their work
	 * 
	 * @return a result name in case the project was not found or the current
	 *         user does not have read permissions for it. null otherwise.
	 */
	 ...
	 * 
	 * @param roleKey
	 *            key for the resource string for the name of the permission
	 *            being checked
	...
	/**
	 * @param orphan
	 *            object orphaned during this operation
	 */
