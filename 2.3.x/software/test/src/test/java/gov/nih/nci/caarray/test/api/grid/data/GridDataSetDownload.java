...
//============================================================================
package gov.nih.nci.caarray.test.api.grid.data;
 ...
 */
public class GridDataSetDownload extends AbstractApiTest {
    ...
    public void testDownloadDataSet() {
        try {
            ...
            for (String experimentName : EXPERIMENT_NAMES) {
                ...
                downloadDataSet(client, experimentName, quantitationTypesCsv);
            }
    ...
    private Object searchById(CaArraySvcClient client, String targetName, Long id) throws RemoteException {
        CQLQuery cqlQuery = new CQLQuery();
        gov.nih.nci.cagrid.cqlquery.Object target = new gov.nih.nci.cagrid.cqlquery.Object();
        ...
        idAttribute.setPredicate(Predicate.EQUAL_TO);
        target.setAttribute(idAttribute);
        cqlQuery.setTarget(target);
        CQLQueryResults cqlResults = client.query(cqlQuery);
        logForSilverCompatibility(API_CALL, "Grid query(CQLQuery)");
        CQLQueryResultsIterator iter = new CQLQueryResultsIterator(cqlResults, CaArraySvcClient.class
