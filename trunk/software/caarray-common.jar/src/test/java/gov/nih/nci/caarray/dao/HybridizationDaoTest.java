...
public class HybridizationDaoTest extends AbstractDaoTest {
    ...
    public void testGetWithNoArrayDesign() throws Exception {
        ...
        PageSortParams<Hybridization> params = new PageSortParams<Hybridization>(5, 0,
                new AdHocSortCriterion<Hybridization>("name"), false);
        HybridizationSearchCriteria criteria = new HybridizationSearchCriteria();        
        criteria.setExperiment(e);
      
        List<Hybridization> hybs = DAO_OBJECT.searchByCriteria(params, criteria);
        assertEquals(2, hybs.size());
        assertEquals(h1.getName(), hybs.get(0).getName());
        assertEquals(h2.getName(), hybs.get(1).getName());

        criteria.getNames().addAll(Arrays.asList("h1"));
        hybs = DAO_OBJECT.searchByCriteria(params, criteria);
