...
public class IlluminaSampleProbeProfileHandlerIntegrationTest extends AbstractServiceIntegrationTest {
    ...
    private AbstractArrayDataServiceTest setupHelper() throws Exception {
        final AbstractArrayDataServiceTest helper = new AbstractArrayDataServiceTest() {
            @Override
    ...
    public void test1() throws Exception {
        ...
        final AbstractArrayDataServiceTest helper = setupHelper();

        final CaArrayFile f = helper.getDataCaArrayFile(IlluminaArrayDataFiles.SAMPLE_PROBE_PROFILE,
        ...
        final FileValidationResult result = helper.arrayDataService.validate(f, null, false);
        ...
        final List<AbstractDesignElement> l = hyb.getDerivedDataCollection().iterator().next().getDataSet()
                .getDesignElementList().getDesignElements();
        ...
        final List<HybridizationData> hdl = hyb.getDerivedDataCollection().iterator().next().getDataSet()
    ...
    public void testValidationErrors_3() {
        final String[] header = { "ID_REF", "Hyb1.DETECTION PVAL", "Hyb1.AVG_Signal", "DETECTION PVAL-Hyb2",
                "AVG_Signal-Hyb2" };
        final String[] messages = { "Mixed column name formats",
    ...
    public void testValidationErrors_4() {
        final String[] header = { "ID_REF", "Hyb1.DETECTION PVAL", "Hyb1.AVG_Signal", "Hyb2.DETECTION PVAL",
                "Hyb2.AVG_Signal", "Hyb2.Foo" };
        final String[] messages = { "Unsupported column Hyb2.Foo", "Ignored column Hyb2.Foo" };
        final SampleProbeProfileHandler.ValidatingHeaderParser p = processHeader(header, messages, null,
                BgxDesignHandler.BGX_FILE_TYPE);
    ...
    public void testInterleavedColumns() {
        final String[] header = { "Hyb2.AVG_Signal", "Hyb1.DETECTION PVAL", "ID_REF", "Hyb1.AVG_Signal",
                "Hyb2.DETECTION PVAL", };
    ...
    public void testValidationErrors_AllQTypes() {
        ...
        final String[] messages = { "QuantitationType DETECTION already defined in column 9",// dup
                                                                                             // "Hyb1.Detection Pval"
        ...
        assertTrue(p.getHybNames().contains("Hyb1"));
        final EnumSet<SampleProbeProfileQuantitationType> expected = EnumSet
                .allOf(SampleProbeProfileQuantitationType.class);
    ...
    static private SampleProbeProfileHandler.ValidatingHeaderParser processHeader(String[] headerColumns,
        ...
        if (sdrf != null) {
            ...
            for (final String hn : sdrf) {
                final gov.nih.nci.caarray.magetab.sdrf.Hybridization h = new gov.nih.nci.caarray.magetab.sdrf.Hybridization();
                h.setName(hn);
                docSet.getSdrfHybridizations().put(hn, Collections.singletonList(h));
            }
            docSet.getSdrfDocuments().add(doc);
        }
        final SampleProbeProfileHandler.ValidatingHeaderParser proc = new SampleProbeProfileHandler.ValidatingHeaderParser(
                designType, result, docSet);
