...
<ul class="checklist" style="width:50%;">
    <@s.iterator value="parameters.list">
        ...
        <li>
            <a name="${parameters.name}-${selectedCount}"></a><label for="${parameters.name}-${selectedCount}"><input id="${parameters.name}-${selectedCount}" name="${parameters.name}" value="${itemKey?html}" type="checkbox"/> ${itemValue?html}</label>
        </li>
    </@s.iterator>
</ul>
