    ...
    %}
        <tr class="template-upload fade">
            ...
            {% } else if (o.files.valid && !i) { %}
                ...
                <td class="start">
                    {% if (!o.options.autoUpload) { %}<button class="btn btn-gray"><i class="icon-upload icon-white"></i>Start</button>{% } %}
                </td>
            {% } else { %}
                <td colspan="2"></td>
            {% } %}
            <td class="cancel">
                {% if (!i) { %}<button class="btn btn-gray"><i class="icon-ban-circle icon-white"></i>Cancel</button>{% } %}
            </td>
        </tr>
    {% } %}
</script>
