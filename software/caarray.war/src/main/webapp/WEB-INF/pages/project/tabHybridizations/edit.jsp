...
<caarray:tabPane subtab="true">
    <caarray:projectListTabItemForm entityName="Hybridization" item="${currentHybridization}" itemName="${currentHybridization.name}"
        isSubtab="true">
        <c:if test="${editMode}">
            <s:textfield name="currentHybridization.name" key="experiment.hybridizations.name" requiredLabel="true" size="80" tabindex="1" />
        </c:if>
        ...
        <s:if test="project.experiment.arrayDesigns.size > 1">
            <s:select requiredLabel="true" key="currentHybridization.array.design" tabindex="4"
