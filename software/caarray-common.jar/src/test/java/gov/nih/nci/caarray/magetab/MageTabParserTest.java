...
public class MageTabParserTest extends AbstractCaarrayTest {
    ...
    private final MageTabParser parser = MageTabParser.INSTANCE;
    ...
    static {
        ...
        final TermSource cto = new TermSource("CTO", "http://obo.sourceforge.net/cgi-bin/detail.cgi?cell", null);
        final TermSource ncbitax = new TermSource("ncbitax", "http://www.ncbi.nlm.nih.gov/Taxonomy/taxonomyhome.html/",
                null);
        final TermSource arrayExpress = new TermSource("ArrayExpress", "http://www.ebi.ac.uk/arrayexpress/", null);
        ...
        CAARRAY1X_MO = mo = new TermSource("MO", "http://mged.sourceforge.net/ontologies/MGEDOntology1.1.8.daml",
                "1.1.8");
        final TermSource adultMouseAnatomy = new TermSource("DB:Adult_Mouse_Anatomy");
        final TermSource nciThesaurus = new TermSource("DB:NCI_Thesaurus");
        final TermSource nciOrganismPartsDb = new TermSource("DB:NCI_organism_part_database");
    ...
    private static void addExpectedFactor(Map<String, ExperimentalFactor> expectedMap, String name, String type,
        ...
        final ExperimentalFactor ef = new ExperimentalFactor();
        ef.setName(name);
    ...
    private static void addExpectedTerm(Map<String, OntologyTerm> expectedMap, String termCategory, String termValue,
        ...
        final OntologyTerm term = createTerm(termCategory, termValue, termSource);
    ...
    private static OntologyTerm createTerm(String category, String value, TermSource ts) {
        final OntologyTerm type = new OntologyTerm();
    ...
    private ValidationResult validate(MageTabFileSet fileSet) throws MageTabParsingException {
        try {
            final MageTabDocumentSet docSet = this.parser.parse(fileSet);
    ...
    public void testValidate() throws MageTabParsingException {
        final MageTabFileSet fileSet = TestMageTabSets.MAGE_TAB_ERROR_SPECIFICATION_INPUT_SET;
        final ValidationResult result = validate(fileSet);
        ...
        assertTrue(result.getMessages().toString()
                .contains("ERROR: Referenced Factor Name EF1 was not found in the IDF"));
        assertTrue(result.getMessages().toString().contains("ERROR: Experimental Factors must have a non-empty name"));
        assertTrue(result.getMessages().toString()
                .contains("Array Design File not allowed via experiment data import."));
    }

    @Test
    public void testSelectRefFiles() throws MageTabParsingException, InvalidDataException {
        final MageTabFileSet fileSet = TestMageTabSets.MAGE_TAB_SPECIFICATION_INPUT_SET;
        final MageTabDocumentSet mTabDocSet = this.parser.parseDataFileNames(fileSet);
    ...
    public void testValidateIllegalUnitPlacement() throws MageTabParsingException {
        final MageTabFileSet fileSet = TestMageTabSets.MAGE_TAB_GEDP_INPUT_SET;
        final ValidationResult result = validate(fileSet);
        assertFalse(result.isValid());
    ...
    public void testParseEmptySet() throws InvalidDataException, MageTabParsingException {
        ...
        final MageTabDocumentSet documentSet = this.parser.parse(inputFileSet);
    ...
    public void testValidateMissingSdrf() throws MageTabParsingException {
        ...
        final ValidationResult result = validate(inputFileSet);
        assertNotNull(result);
        assertFalse(result.isValid());
        assertEquals(1, result.getFileValidationResults().size());
        final FileValidationResult fileValidationResult = result
                .getFileValidationResult(MageTabDataFiles.SPECIFICATION_EXAMPLE_IDF.getName());
        assertNotNull(fileValidationResult);
    ...
    public void testValidateMissingIdf() throws MageTabParsingException {
        ...
        assertEquals(1, result.getFileValidationResults().size());
        final FileValidationResult fileValidationResult = result
                .getFileValidationResult(MageTabDataFiles.SPECIFICATION_EXAMPLE_SDRF.getName());
        assertNotNull(fileValidationResult);
    ...
    public void testValidateMisplacedFactorValues() throws MageTabParsingException {
        final MageTabFileSet inputFileSet = TestMageTabSets.MAGE_TAB_MISPLACED_FACTOR_VALUES_INPUT_SET;
        final ValidationResult result = validate(inputFileSet);
        assertNotNull(result);
        assertFalse(result.isValid());
        assertEquals(3, StringUtils.countMatches(result.toString(),
                "Factor Value columns must come after (to the right of) a Hybridization column"));
    ...
    public void testParseTcgaBroadDocuments() throws MageTabParsingException, InvalidDataException {
        final MageTabFileSet fileSet = TestMageTabSets.TCGA_BROAD_INPUT_SET;
        final MageTabDocumentSet documentSet = this.parser.parse(fileSet);
        assertNotNull(documentSet);
        assertEquals(1, documentSet.getIdfDocuments().size());
        assertEquals(-1,
                documentSet.getIdfDocuments().iterator().next().getInvestigation().getDescription().indexOf("<"));
        assertEquals(-1,
                documentSet.getIdfDocuments().iterator().next().getInvestigation().getDescription().indexOf(">"));
    ...
    public void testDefect12537() throws MageTabParsingException, InvalidDataException {
        ...
        final ValidationResult validationResult = validate(fileSet);
        assertFalse(validationResult.isValid());
        final List<ValidationMessage> errors = validationResult.getMessages(ValidationMessage.Type.ERROR);
        ...
        final SdrfDocument sdrfDocument = documentSet.getSdrfDocuments().iterator().next();
        assertEquals(2, sdrfDocument.getAllDerivedArrayDataMatrixFiles().size());
        assertEquals(3, sdrfDocument.getAllHybridizations().size());
        for (final Hybridization hyb : sdrfDocument.getAllHybridizations()) {
    ...
    public void testParseCarray1xDocuments() throws MageTabParsingException, InvalidDataException {
        final MageTabFileSet fileSet = TestMageTabSets.CAARRAY1X_INPUT_SET;
        final MageTabDocumentSet documentSet = this.parser.parse(fileSet);
        assertNotNull(documentSet);
        assertTrue(documentSet.getValidationResult().isValid());
        assertEquals(1, documentSet.getIdfDocuments().size());
        ...
        final IdfDocument idfDocument = documentSet.getIdfDocuments().iterator().next();
        final SdrfDocument sdrfDocument = documentSet.getSdrfDocuments().iterator().next();
    ...
    private void checkProtocols(Map<String, Protocol> expectedProtocols, Collection<Protocol> protocols) {
        ...
        for (final Protocol protocol : protocols) {
            final Protocol expectedProtocol = expectedProtocols.get(protocol.getName());
    ...
    private void checkTermSources(Map<String, TermSource> expectedTermSources, Collection<TermSource> termSources) {
        ...
        for (final TermSource termSource : termSources) {
            final TermSource expectedTermSource = expectedTermSources.get(termSource.getName());
    ...
    private void checkFactors(Map<String, ExperimentalFactor> expectedFactors, Collection<ExperimentalFactor> factors) {
        ...
        for (final ExperimentalFactor factor : factors) {
            final ExperimentalFactor expectedFactor = expectedFactors.get(factor.getName());
    ...
    private void checkFactorsMatch(ExperimentalFactor expectedFactor, ExperimentalFactor factor) {
        ...
        assertTrue("Expected factor " + ToStringBuilder.reflectionToString(expectedFactor)
                + " differs from actual factor" + ToStringBuilder.reflectionToString(factor),
                EqualsBuilder.reflectionEquals(expectedFactor, factor, new String[] { "type" }));
    }
    ...
    private void checkPersons(Map<String, Person> expectedPersons, Collection<Person> persons) {
        ...
        for (final Person person : persons) {
            final Person expectedPerson = expectedPersons.get(personKey(person));
    ...
    private void checkPersonsMatch(Person expectedPerson, Person person) {
        ...
        assertTrue("Expected person " + ToStringBuilder.reflectionToString(expectedPerson)
                + " differs from actual person" + ToStringBuilder.reflectionToString(person),
                EqualsBuilder.reflectionEquals(expectedPerson, person, new String[] { "roles" }));
    }
    ...
    private void checkPublications(Map<String, Publication> expectedPublications, Collection<Publication> publications) {
        ...
        for (final Publication publication : publications) {
            final Publication expectedPublication = expectedPublications.get(publicationKey(publication));
    ...
    private void checkPublicationsMatch(Publication expectedPublication, Publication publication) {
        ...
        assertTrue("Expected publication " + ToStringBuilder.reflectionToString(expectedPublication)
                + " differs from actual publication" + ToStringBuilder.reflectionToString(publication),
                EqualsBuilder.reflectionEquals(expectedPublication, publication, new String[] { "status" }));
    }
    ...
    private void checkTerms(Map<String, OntologyTerm> expectedTerms, Collection<OntologyTerm> terms) {
        ...
        for (final OntologyTerm term : terms) {
            final OntologyTerm expectedTerm = expectedTerms.get(term.getValue());
    ...
    private void checkCarray1xSdrfDocument(SdrfDocument sdrfDocument) {
        ...
        final Source source = sdrfDocument.getAllSources().get(0);
        ...
        checkTermsMatch(
                createTerm(MageTabOntologyCategory.MATERIAL_TYPE.getCategoryName(), "organism_part", CAARRAY1X_MO),
                source.getMaterialType());
        assertEquals(2, source.getProtocolApplications().size());
        assertEquals(36, sdrfDocument.getAllSamples().size());
        final Sample sample = sdrfDocument.getAllSamples().get(0);
        ...
        checkTermsMatch(
                createTerm(MageTabOntologyCategory.MATERIAL_TYPE.getCategoryName(), "synthetic_RNA", CAARRAY1X_MO),
                sample.getMaterialType());
        assertEquals(38, sdrfDocument.getAllLabeledExtracts().size());
        final LabeledExtract le = sdrfDocument.getAllLabeledExtracts().get(0);
        assertEquals("Cy3 labeled Pr111 reference_8kNew111_14_4601_m84", le.getName());
        checkTermsMatch(
                createTerm(MageTabOntologyCategory.MATERIAL_TYPE.getCategoryName(), "synthetic_RNA", CAARRAY1X_MO),
                le.getMaterialType());
        checkTermsMatch(createTerm(MageTabOntologyCategory.LABEL_COMPOUND.getCategoryName(), "Cy3", null),
                le.getLabel());
        ...
        final Hybridization hyb = sdrfDocument.getAllHybridizations().get(0);
        ...
        final DerivedArrayDataFile arrayDataFile = hyb.getSuccessorDerivedArrayDataFiles().iterator().next();
    ...
    private void checkSdrfTranslation(SdrfDocument document) {
        final List<Hybridization> hybridizations = document.getAllHybridizations();
        ...
        final List<ArrayDataFile> arrayDataFiles = document.getAllArrayDataFiles();
    ...
    private void checkArrayDesigns(MageTabDocumentSet documentSet) {
        ...
        final ArrayDesign arrayDesign = sdrfDocument.getAllArrayDesigns().get(0);
        for (final Hybridization hybridization : sdrfDocument.getAllHybridizations()) {
    ...
    public void testSourceDescriptionTranslation() throws InvalidDataException, MageTabParsingException {
        final MageTabFileSet fileSet = TestMageTabSets.MAGE_TAB_UNSUPPORTED_DATA_INPUT_SET;
        final MageTabDocumentSet documentSet = this.parser.parse(fileSet);
        final SdrfDocument sdrfDocument = documentSet.getSdrfDocuments().iterator().next();
    ...
    public void testParseSpecificationDocuments() throws MageTabParsingException, InvalidDataException {
        ...
        checkTerms(documentSet);
        assertEquals(1, documentSet.getIdfDocuments().size());
        final IdfDocument idfDocument = documentSet
                .getIdfDocument(MageTabDataFiles.SPECIFICATION_EXAMPLE_IDF.getName());
        ...
        final Source firstSource = sdrfDocument.getAllSources().get(0);
        ...
        final Hybridization firstHybridization = sdrfDocument.getAllHybridizations().get(0);
        final Set<ArrayDataFile> rawDataFiles = firstHybridization.getSuccessorArrayDataFiles();
        assertEquals(2, rawDataFiles.size());
        final Iterator<ArrayDataFile> rawDataIterator = rawDataFiles.iterator();
    ...
    public void testParseDerivedData() throws Exception {
        final MageTabFileSet fileSet = TestMageTabSets.DERIVED_DATA_INPUT_SET;
        final MageTabDocumentSet documentSet = this.parser.parse(fileSet);
        assertNotNull(documentSet);
        final SdrfDocument sdrfDocument = documentSet.getSdrfDocuments().iterator().next();
        assertEquals(1, sdrfDocument.getAllArrayDataFiles().size());
        final ArrayDataFile arrayDataFile = sdrfDocument.getAllArrayDataFiles().get(0);
        final List<DerivedArrayDataFile> derivedDataFiles = sdrfDocument.getAllDerivedArrayDataFiles();
        assertEquals(2, derivedDataFiles.size());

        // make sure the first derived data file is derived from the raw data
        final DerivedArrayDataFile derivedArrayDataFile = derivedDataFiles.get(0);
        final Set<ArrayDataFile> predecessorArrayDataFiles = derivedArrayDataFile.getPredecessorArrayDataFiles();
        ...
        final DerivedArrayDataFile derivedArrayDataFile2 = derivedDataFiles.get(1);
        final Set<DerivedArrayDataFile> predecessorDerivedArrayDataFiles = derivedArrayDataFile2
                .getPredecessorDerivedArrayDataFiles();
    ...
    private void checkTerms(MageTabDocumentSet documentSet) {
        for (final OntologyTerm term : documentSet.getTerms()) {
    ...
    public void testFeature13141() throws Exception {
        final MageTabFileSet fileSet = TestMageTabSets.VALID_FEATURE_13141_INPUT_SET;
        final MageTabDocumentSet documentSet = this.parser.parse(fileSet);
        assertNotNull(documentSet);
    ...
    private void verifyFeature13141Sample(Sample s1, String expectedSampleName, String expectedTermValue) {
        ...
        assertEquals(ExperimentOntologyCategory.EXTERNAL_SAMPLE_ID.getCategoryName(), s1.getCharacteristics().get(0)
                .getCategory());
    ...
    private void verifyFeature13141Source(Source s1, String expectedName, String expectedValue) {
        ...
        assertEquals(1, s1.getCharacteristics().size());
        assertEquals(ExperimentOntologyCategory.EXTERNAL_ID.getCategoryName(), s1.getCharacteristics().get(0)
                .getCategory());
    ...
    public void testFeature13141VerifyCanParsesInvalidSdrf() throws Exception {
        final MageTabFileSet fileSet = TestMageTabSets.INVALID_FEATURE_13141_INPUT_SET;
        final MageTabDocumentSet documentSet = this.parser.parse(fileSet);
        assertNotNull(documentSet);
        assertEquals(2, documentSet.getSdrfDocuments().size());
    ...
    public void testDefect16421() throws Exception {
        final MageTabFileSet fileSet = TestMageTabSets.DEFECT_16421;
        final ValidationResult validationResult = validate(fileSet);
        assertTrue(validationResult.isValid());

        final MageTabFileSet fileSet2 = TestMageTabSets.DEFECT_16421_2;
        final ValidationResult validationResult2 = validate(fileSet2);
        final List<ValidationMessage> errors = validationResult2.getMessages(ValidationMessage.Type.ERROR);
    ...
    public void testInvalidNodeOrder() throws Exception {
        final MageTabFileSet fs = TestMageTabSets.INVALID_NODE_ORDER_SET;
        final ValidationResult vr = validate(fs);
        final List<ValidationMessage> errors = vr.getMessages(ValidationMessage.Type.ERROR);
    ...
    public void testMissingColumns() throws Exception {
        for (final MageTabFileSet fs : Arrays.asList(TestMageTabSets.NO_BIOMATERIAL_SET,
    ...
    public void testExtendedFactorValues() throws Exception {
        final MageTabFileSet fileSet = TestMageTabSets.EXTENDED_FACTOR_VALUES_INPUT_SET;
        ...
        try {
            ...
            verifyExtendedFactorValuesSampleChars(sdrfDocument.getAllSamples().get(1), "Sample B", "234", "a lot",
                    "months", "tissue", "MO");
            verifyExtendedFactorValuesSampleChars(sdrfDocument.getAllSamples().get(2), "Sample C", "345", "2.2",
                    "days", "unknown", null);
            verifyExtendedFactorValuesSampleParams(sdrfDocument.getAllSamples().get(0), "Sample A", "foo", "planting",
                    "MO", "4", "kg");
            verifyExtendedFactorValuesSampleParams(sdrfDocument.getAllSamples().get(1), "Sample B", "baz", "planting",
                    "MO", "4", null);
            verifyExtendedFactorValuesSampleParams(sdrfDocument.getAllSamples().get(2), "Sample C", "foo", "nothing",
                    null, "less", "mg");
            assertEquals(3, sdrfDocument.getAllHybridizations().size());
            verifyExtendedFactorValuesHyb(sdrfDocument.getAllHybridizations().get(0), "Hyb A", "123", "5", "years",
                    "tissue", "MO");
            verifyExtendedFactorValuesHyb(sdrfDocument.getAllHybridizations().get(1), "Hyb B", "234", "a lot",
                    "months", "tissue", "MO");
    ...
    private void verifyExtendedFactorValuesSampleParams(Sample sample, String sampleName, String pv1Value,
            String pv2Value, String pv2ts, String pv3Value, String pv3Unit) {
        assertEquals(sampleName, sample.getName());
        assertEquals(1, sample.getProtocolApplications().size());
        final ProtocolApplication pa = sample.getProtocolApplications().get(0);
        ...
        assertEquals(pv3Unit,
                pv3Unit == null ? pa.getParameterValues().get(2).getUnit() : pa.getParameterValues().get(2).getUnit()
                        .getValue());
        if (pv3Unit != null) {
